...
use axiom::{
    ...
    ipc::{AxiomIPCServer, LazyUIMessage, AxiomMessage}
};
...
async fn test_ipc_message_protocol() -> Result<()> {
    ...
    let optimize_json = r#"{"type":"OptimizeConfig","changes":{"blur_radius":5.0},"reason":"performance"}"#;
...
async fn test_compositor_initialization() -> Result<()> {
    ...
    let result = timeout(
        ...
        AxiomCompositor::new(config, true)
    ).await;
...
async fn test_input_processing() -> Result<()> {
    use axiom::input::{InputManager, InputEvent, CompositorAction};
...
async fn test_concurrent_operations() -> Result<()> {
    ...
    let workspaces = Arc::new(Mutex::new(
        ScrollableWorkspaces::new(&config)?
    ));
