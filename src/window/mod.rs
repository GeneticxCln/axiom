...
impl WindowManager {
    ...
    pub fn calculate_layout(&self, workspace_bounds: Rectangle) -> Vec<(u64, Rectangle)> {
        let windows_in_workspace: Vec<_> = self.windows.values()
        ...
        match self.config.default_layout.as_str() {
            "horizontal" => {
                ...
                let available_width = workspace_bounds.width as i32 - (gap * (windows_in_workspace.len() as i32 + 1));
            ...
            "vertical" => {
                ...
                let available_height = workspace_bounds.height as i32 - (gap * (windows_in_workspace.len() as i32 + 1));
