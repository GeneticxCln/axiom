...
use crate::window::{AxiomWindow, Rectangle};
...
impl ScrollableWorkspaces {
    pub fn new(config: &WorkspaceConfig) -> Result<Self> {
        ...
        debug!("📏 Workspace width: {} px, gaps: {} px", 
    ...
    pub fn ensure_column(&mut self, index: i32) -> &mut WorkspaceColumn {
        if !self.columns.contains_key(&index) {
            ...
            debug!("📄 Created new workspace column {} at position {}", index, position);
    ...
    pub fn scroll_to_column(&mut self, column_index: i32) {
        ...
        info!("📱 Scrolling to column {} (position: {:.1})", column_index, target_pos);
    }
    ...
    pub fn start_momentum_scroll(&mut self, velocity: f64) {
        if velocity.abs() > 10.0 { // Minimum velocity threshold
    ...
    pub fn remove_window(&mut self, window_id: u64) -> Option<i32> {
        for (column_index, column) in self.columns.iter_mut() {
            if column.remove_window(window_id) {
                debug!("🗑️ Removed window {} from column {}", window_id, column_index);
    ...
    pub fn move_window_to_column(&mut self, window_id: u64, target_column: i32) -> bool {
        ...
        if let Some(_current_column) = self.remove_window(window_id) {
        ...
        match self.scroll_state {
            ScrollState::Scrolling { 
                ...
                if elapsed >= duration {
                    ...
                    debug!("✅ Scroll animation completed at position {:.1}", target_position);
                } else {
                    ...
                    self.current_position = start_position + 
                        (target_position - start_position) * eased_progress;
                        
                    // Calculate velocity for smooth transitions
                    self.scroll_velocity = (target_position - start_position) * 
                        self.ease_out_cubic_derivative(progress) / duration.as_secs_f64();
                }
            },
            
            ScrollState::Momentum { start_time, start_position, velocity } => {
    ...
    fn cleanup_empty_columns(&mut self) {
        ...
        let columns_to_remove: Vec<i32> = self.columns
            .iter()
            .filter(|(index, column)| {
    ...
    pub fn scroll_progress(&self) -> f64 {
        match self.scroll_state {
            ScrollState::Scrolling { start_time, duration, .. } => {
