...
async fn demonstrate_preserved_systems(compositor: &mut AxiomCompositor) -> Result<()> {
    ...
    info!("  📊 Current state: Column {}, Position {:.1}, {} total, Scrolling: {}", 
    ...
    info!("  ✅ Workspace system working! New position: Column {}, Position {:.1}", 
...
async fn demonstrate_wayland_functionality() -> Result<()> {
    ...
    if let Ok(wayland_display) = std::env::var("WAYLAND_DISPLAY") {
        info!("✅ WAYLAND_DISPLAY environment variable: {}", wayland_display);
