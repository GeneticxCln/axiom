...
impl Default for TitlebarButtons {
    fn default() -> Self {
        Self {
            close: ButtonState::default(),
            minimize: ButtonState::default(),
            maximize: ButtonState::default(),
        }
    }
}
...
impl Default for ButtonState {
    fn default() -> Self {
        Self {
            ...
            hovered: false,
            pressed: false,
            bounds: Rectangle { x: 0, y: 0, width: 24, height: 24 },
        }
    }
}
...
impl DecorationManager {
    ...
    pub fn add_window(&mut self, window_id: u64, title: String, prefers_server_side: bool) {
        ...
        debug!("🪟 Added decoration for window {} (mode: {:?})", window_id, mode);
    }
    ...
    pub fn set_window_focus(&mut self, window_id: u64, focused: bool) {
        if let Some(decoration) = self.decorations.get_mut(&window_id) {
            if decoration.focused != focused {
                ...
                debug!("🎯 Window {} focus: {}", window_id, if focused { "gained" } else { "lost" });
            }
        }
    }
    ...
    pub fn set_decoration_mode(&mut self, window_id: u64, mode: DecorationMode) {
        if let Some(decoration) = self.decorations.get_mut(&window_id) {
            if decoration.mode != mode {
                ...
                info!("🎨 Changed decoration mode for window {} to {:?}", window_id, mode);
            }
        }
            ...
            match decoration.mode {
                DecorationMode::ServerSide => {
                    ...
                    Rectangle {
                        ...
                        height: window_rect.height.saturating_sub(
                            decoration.titlebar_height + (border_width * 2) as u32
                        ),
                    }
                },
            ...
            match decoration.mode {
                DecorationMode::ServerSide => {
                    ...
                    Rectangle {
                        ...
                        height: content_rect.height + decoration.titlebar_height + (border_width * 2) as u32,
                    }
                },
    ...
    pub fn handle_button_press(&mut self, window_id: u64, x: i32, y: i32) -> Option<DecorationAction> {
        if let Some(decoration) = self.decorations.get_mut(&window_id) {
            if decoration.mode != DecorationMode::ServerSide {
                return None;
            }
    ...
    pub fn handle_mouse_motion(&mut self, window_id: u64, x: i32, y: i32) {
        if let Some(decoration) = self.decorations.get_mut(&window_id) {
            ...
            decoration.buttons.minimize.hovered = decoration.buttons.minimize.bounds.contains_point(x, y);
            decoration.buttons.maximize.hovered = decoration.buttons.maximize.bounds.contains_point(x, y);
        }
    }
    ...
    pub fn render_decorations(&self, window_id: u64, window_rect: Rectangle, effects: Option<&WindowEffectState>) -> Result<DecorationRenderData> {
        let decoration = self.decorations.get(&window_id)
            .ok_or_else(|| anyhow::anyhow!("Window {} not found in decoration manager", window_id))?;
        ...
        let render_data = DecorationRenderData::ServerSide {
            ...
            titlebar_bg: [titlebar_bg[0], titlebar_bg[1], titlebar_bg[2], titlebar_bg[3] * opacity],
            border_width,
            border_color: [border_color[0], border_color[1], border_color[2], border_color[3] * opacity],
            corner_radius,
            title: decoration.title.clone(),
            text_color: [text_color[0], text_color[1], text_color[2], text_color[3] * opacity],
            font_size: self.theme.font_size,
            buttons: decoration.buttons.clone(),
        };
        
        debug!("🎨 Generated decoration render data for window {}", window_id);
