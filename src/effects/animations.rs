...
impl AnimationController {
    ...
    pub fn start_spring_animation(
        ...
        self.spring_states.insert((window_id, property_name.clone()), spring_state);
        
        debug!("🌸 Started spring animation for window {} property '{}': {} -> {}",
               window_id, property_name, current_value.unwrap_or(0.0), target_value);
    }
    ...
    pub fn start_timeline(&mut self, timeline: AnimationTimeline) -> Result<()> {
        ...
        self.active_timelines.insert(timeline_name.clone(), timeline_state);
        
        info!("🎭 Started animation timeline '{}' with {} events",
              timeline_name, self.timelines[&timeline_name].keyframes.len());
    ...
    pub fn update(&mut self) -> Result<Vec<AnimationUpdate>> {
        ...
        self.animation_count = self.active_animations.values()
            .map(|anims| anims.len())
            .sum::<usize>() + self.spring_states.len();
        
        if !updates.is_empty() {
            debug!("🎬 Animation update: {} changes, {} active animations",
                   updates.len(), self.animation_count);
        }
        ...
        for ((window_id, property_name), spring_state) in self.spring_states.iter_mut() {
            ...
            if displacement.abs() < spring_state.params.precision && 
               spring_state.velocity.abs() < velocity_threshold {
                ...
                debug!("🌸 Spring animation settled for window {} property '{}'",
    ...
    fn update_timelines(&mut self, now: Instant, _updates: &mut Vec<AnimationUpdate>) -> Result<()> {
        ...
        for timeline_name in timeline_names {
            ...
            let progress = elapsed_in_timeline.as_secs_f64() / timeline.total_duration.as_secs_f64();
            ...
            for (event_index, event) in timeline.keyframes.iter().enumerate() {
                ...
                if progress >= event_start_progress as f64 && progress < event_end_progress as f64 {
                    ...
                    let already_active = timeline_state.active_events
                        .iter()
                    ...
                    if !already_active {
                        ...
                        timeline_state.active_events.push((event_index, active_animation));
                    }
                }
            }
    ...
    fn calculate_animation_value(&self, animation: &AnimationType, progress: f32) -> Option<(AnimationProperty, AnimationValue)> {
        match animation {
            AnimationType::WindowOpen { target_scale, target_opacity, .. } => {
    ...
    fn cleanup_finished_animations(&mut self) {
        ...
        self.spring_states.retain(|_, spring_state| !spring_state.settled);
    }
    ...
    pub fn set_global_speed(&mut self, speed: f32) {
        ...
        info!("⚡ Animation speed set to {:.1}x", self.global_speed_multiplier);
    }
