...
impl ShadowRenderer {
    ...
    /// ## Known limitation (L2)
    ///
    /// Each shadow allocates its own `Buffer` via `create_buffer_init`
    /// on every frame, even though they share the same render pass and
    /// pipeline. A further optimisation would pack all shadow uniforms
    /// into a single GPU buffer and use **dynamic uniform offsets**
    /// (`has_dynamic_offset: true` on the bind group layout entry,
    /// `&[offset]` in `set_bind_group`) so the per-frame allocation
    /// count is O(1) instead of O(N). This was deferred because:
    ///
    /// 1. The wgpu bind-group layout for the drop-shadow shader would
    ///    need to be rebuilt with `has_dynamic_offset: true`, which
    ///    changes the pipeline layout and forces a render-pipeline
    ///    rebuild.
    /// 2. The compositor currently routes shadows through a GL→WGPU
    ///    bridge that already serialises the full framebuffer; the
    ///    shadow uniform buffer allocations are dwarfed by that cost.
    /// 3. Shadow count per frame is typically low (< 10 windows),
    ///    making the O(N) allocation overhead negligible in practice.
    ///
    /// When a pure-WGPU rendering path lands (no GL bridge), or when
    /// shadow count routinely exceeds 20, this should be revisited.
