...
impl InputManager {
    pub fn new(input_config: &InputConfig, bindings_config: &BindingsConfig) -> Result<Self> {
        ...
        key_bindings.insert(bindings_config.scroll_left.clone(), CompositorAction::ScrollWorkspaceLeft);
        key_bindings.insert(bindings_config.scroll_right.clone(), CompositorAction::ScrollWorkspaceRight);
        key_bindings.insert(bindings_config.move_window_left.clone(), CompositorAction::MoveWindowLeft);
        key_bindings.insert(bindings_config.move_window_right.clone(), CompositorAction::MoveWindowRight);
    ...
    pub fn process_input_event(&mut self, event: InputEvent) -> Vec<CompositorAction> {
        match event {
            InputEvent::Keyboard { key, modifiers, pressed } => {
            ...
            InputEvent::MouseMove { x, y, delta_x: _, delta_y: _ } => {
            ...
            InputEvent::Gesture { gesture_type, delta_x, delta_y, velocity } => {
    ...
    fn process_keyboard_event(&mut self, key: String, modifiers: Vec<String>, pressed: bool) -> Vec<CompositorAction> {
        if pressed {
    ...
    fn process_mouse_button(&mut self, button: MouseButton, pressed: bool, x: f64, y: f64) -> Vec<CompositorAction> {
        self.mouse_position = (x, y);
        
        if pressed {
            debug!("🐁 Mouse button {:?} pressed at ({:.1}, {:.1})", button, x, y);
    ...
    fn process_scroll_event(&mut self, _x: f64, _y: f64, delta_x: f64, delta_y: f64) -> Vec<CompositorAction> {
    ...
    fn process_gesture_event(&mut self, gesture_type: GestureType, delta_x: f64, delta_y: f64, velocity: f64) -> Vec<CompositorAction> {
        match gesture_type {
            GestureType::Swipe => {
                debug!("👋 Swipe gesture: delta=({:.1}, {:.1}), velocity={:.1}", delta_x, delta_y, velocity);
