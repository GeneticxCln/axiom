...
async fn demo_animation_showcase(compositor: &mut AxiomCompositor) -> Result<()> {
    ...
    for (i, &window_id) in window_ids.iter().enumerate() {
        ...
        debug!(
            "🎬 Started open animation for window {} ({}/{})",
            window_id,
            i + 1,
            window_ids.len()
        );
    ...
    for (i, &window_id) in window_ids.iter().enumerate() {
        ...
        compositor
            .effects_engine_mut()
            .animate_window_move(window_id, from, to);
    ...
    for &window_id in &window_ids[0..2] {
        ...
        for (j, &pos) in positions.iter().enumerate() {
            ...
            compositor
                .effects_engine_mut()
                .animate_window_move(window_id, prev_pos, pos);
            sleep(Duration::from_millis(100)).await;
        }
    }
...
async fn demo_effects_quality(compositor: &mut AxiomCompositor) -> Result<()> {
    ...
    for (quality_name, quality_value) in quality_levels {
        ...
        for (_i, &window_id) in window_ids.iter().enumerate() {
            ...
            if let Some(window_effects) = compositor
                .effects_engine_mut()
                .get_window_effects(window_id)
            {
                debug!(
                    "🌊 Window {} effects: scale={:.2}, opacity={:.2}, corner_radius={:.1}",
                    window_id,
                    window_effects.scale,
                    window_effects.opacity,
                    window_effects.corner_radius
                );
            }
...
async fn demo_performance_adaptation(compositor: &mut AxiomCompositor) -> Result<()> {
    ...
    info!(
        "📊 Created {} windows for performance testing",
        window_ids.len()
    );
    ...
    while start_time.elapsed() < test_duration {
        ...
        let (frame_time, effects_quality, active_effects) =
            compositor.effects_engine_mut().get_performance_stats();

        debug!(
            "⚡ Performance cycle {}: frame_time={:.1}ms, quality={:.2}, effects={}",
            animation_cycle,
        ...
        if animation_cycle % 20 == 0 {
            info!(
                "📈 Performance update: {:.1}ms frame time, {:.1}% quality, {} active effects",
                frame_time.as_secs_f64() * 1000.0,
                effects_quality * 100.0,
                active_effects
            );
        }
    }
...
async fn demo_advanced_features(compositor: &mut AxiomCompositor) -> Result<()> {
    ...
    for i in 0..3 {
        ...
        compositor
            .effects_engine_mut()
            .animate_workspace_transition(from_offset, to_offset);
    ...
    while start_time.elapsed() < optimization_test_duration {
        ...
        let (frame_time, effects_quality, _) =
            compositor.effects_engine_mut().get_performance_stats();

        if frame_time.as_millis() > 20 {
            debug!(
                "⚠️ Frame time high: {:.1}ms, quality adjusted to {:.2}",
                frame_time.as_secs_f64() * 1000.0,
                effects_quality
            );
        }
...
pub fn display_effects_capabilities(compositor: &AxiomCompositor) {
    ...
    let (frame_time, effects_quality, active_effects) =
        compositor.effects_engine().get_performance_stats();

    info!("📊 Current Performance Statistics:");
    info!(
        "  ⏱️  Frame Time: {:.2}ms ({:.1} FPS)",
        frame_time.as_secs_f64() * 1000.0,
