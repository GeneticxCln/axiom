...
impl EnhancedSmithayBackend {
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        info!(
            "✅ Enhanced Smithay backend initialized with Wayland socket: {}",
            socket_name
        );
        Ok(())
    }
    ...
    fn create_wayland_socket(&mut self) -> Result<String> {
        ...
        let listener =
            UnixListener::bind(&socket_path).context("Failed to create Wayland socket")?;
        ...
        if let Some(listener) = &self.listener {
            ...
            match listener.accept() {
                Ok((_stream, addr)) => {
                    ...
                    info!(
                        "👤 New Wayland client connected: {} (id: {})",
                        addr.as_pathname()
...
pub async fn run_enhanced_compositor(
    ...
    while running {
        ...
        if frame_count % 60 == 0 {
            ...
            debug!(
                "📊 Stats: {} clients, {} windows, {:.1}s uptime",
...
/// Convenience wrapper used by the real-compositor feature path that doesn't require
/// the caller to pass a windowed flag.
pub async fn run_enhanced_compositor_default(
    config: crate::config::AxiomConfig,
) -> Result<()> {
    // Default to windowed=false for production; callers that need windowed should
    // invoke run_enhanced_compositor directly.
    run_enhanced_compositor(config, false).await
}
